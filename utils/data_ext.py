...
def convert_example_to_feature(example, tokenizer, label2id,  max_seq_len=512, is_test=False):
    ...
    if not is_test:
        ...
        # print('wwww',example["text"],len(example["text"]))
        # print('aaa',example["label"],len(example["label"]))
        label = [label2id["O"]]+[label2id[label_term] for label_term in example["label"]][:(max_seq_len-2)]+[label2id["O"]]
        a=[label2id[label_term] for label_term in example["label"]][:(max_seq_len-2)]
        # print('bbbb',a,len(a))
        
        if len(encoded_inputs["input_ids"]) != len(label):
            print(example["text"])
            # print('cccc____',encoded_inputs["input_ids"],len(encoded_inputs["input_ids"]))
            # print('cccc____',label,len(label),end='\n\n')
